"""工具 annotations 黄金锁: 17 个工具的 MCP hint 值逐工具锁定 (四项全显式)。
...
四项必须全显式: 目录站/OpenAI directory 对缺任一 hint 的工具拒收, 只读组不依赖
规范缺省 (destructive=true/idempotent=false)。改任何 hint 值 = 元数据 wire 变化:
必须同步 server.py 的 _ANN_* 分组与这里的黄金表, 并在 Release notes 标注。
双向覆盖: 注册集合 == 黄金键集合, 新增工具不登记 annotations 直接挂测试
(分组语义见 server.py _ANN_* 注释)。
"""
...
_GOLDEN: dict[str, dict] = {
    ...
    # 联网只读 (探测/读/浏览/诊断/取帧: 不破坏环境, 重复调用无额外效果)
    "probe_device": {
        "readOnlyHint": True, "destructiveHint": False,
