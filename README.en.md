...
With `PLCTAP_ALLOW_WRITE=true`, all five endpoints are writable (modbus_rtu shares the modbus semantics):
...
Addressing model and common options for the five endpoints (also available at runtime via `list_protocols`):
...
2. `probe_device(protocol, host, port)` — connectivity check; on failure returns four-class layered attribution
   ...
   network route or the protocol configuration.
...
transparent proxy / modbus_rtu endpoint / vendor_hints) are not in the baseline. The detect tier (automatic
protocol detection, 4 cases, added in v0.4) requires live network services for active probing and does not fit
bare Q&A, so it is not in the comparison — tool mode totals 39/39 across six tiers (re-run 2026-09-07 on the
current corpus).
